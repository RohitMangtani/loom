...
# public tunnel URL as NEXT_PUBLIC_WS_URL.
...
if [ ! -f "$TUNNEL_FILE" ]; then
  ...
  echo "Start Hive with 'npm start' first so the daemon can create a public WebSocket URL."
  exit 1
fi
...
if [[ ! "$TUNNEL_URL" =~ ^https:// ]]; then
  echo "Could not parse an HTTPS tunnel URL from $TUNNEL_FILE"
